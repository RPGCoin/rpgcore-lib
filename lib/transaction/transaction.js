...
/**
 * Retrieve a hexa string that can be used with rpgd's CLI interface
 ...
 * * `disableMoreOutputThanInput`: disable checking if the transaction spends more RPGcoins than the sum of the input amounts
 * @return {string}
 */
 ...
 *
 * Can receive, as output information, the output of rpgd's `listunspent` command,
 * and a slightly fancier format recognized by rpgcore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a rpgcore Address object. The
 * same is true for `script`, which can be a string or a rpgcore Script.
 *
 ...
 *
 * // From a pay to public key hash output from rpgd's listunspent
...
/**
 * Analogous to rpgd's IsCoinBase function in transaction.h
 */
